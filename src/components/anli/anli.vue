		...
		<div class="contain">
			<el-row :gutter="15">
				<el-col :lg="6" :md="8" :sm="12" v-for="item in dataList">
					<a :href="baseUrl+item.demoUrl" target="_blank">
					  	<div class="img_wrap">
					  		<div class="img">
					  			<img :src="baseUrl+item.imageUrl" alt="...">
					  		</div>
					  		<div class="ewm_img">
					  			<img :src="'http://qr.topscan.com/api.php?text='+baseUrl+item.demoUrl" alt="...">
					  		</div>
					  		<h3>
...
<script>
	...
	export default {
		...
		mounted () {
			$('#app').css('background-image','url("/static/images/anli_banner.jpg")');//banner图片更改
			//请求文章data
		  	this.$http.get(baseUrl+'/anliContent?articleNum='+this.initPageSize).then(function(res){
		  		this.dataList = res.body;
		  	},function(err){
		  		console.log(err)
		  	});
		  	//请求nav导航data
		  	this.$http.get(baseUrl+'/anliNav').then(function(res){
		  		console.log(res);
		...
		methods:{
			navClick(index,ev){
				...
				if(!isClick){
					...
					this.index = index;
					this.$http.get(baseUrl+'/anliContent?articleNum='+this.pageSize[index]+'&selectValue='+this.navList[index].code)
					.then(function(res){
						  		this.dataList = res.body;
						  		isClick = false;
						  	},function(err){
			...
			getMore(){
				...
				if(!isClick){
					...
					this.$http.get(baseUrl+'/anliContent?articleNum='+this.pageSize[this.index]+'&selectValue='+this.getMoreSelect)
					.then(function(res){
						  		this.dataList = res.body;
						  		isClick = false;
						  	},function(err){
